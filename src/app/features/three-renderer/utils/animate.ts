...
import { EdgesGeometry, LineSegments, Vector3, BufferGeometry, Vector2 } from 'three';
import { Clock } from './clock';
import { satrecToXYZ } from './conversion';
import { TwoLineElement } from './tle';
import * as THREE from 'three';


// Converts a point [longitude, latitude] in degrees to a THREE.Vector3.
export const vertex = (point, radius: number) => {
...
export const wireframe = (multilinestring, material) => {
...
export const graticule10 = () => {
    ...
    const graticuleX = (y0, y1, dy) => {
        var y = d3.range(y0, y1 - epsilon, dy).concat(y1);
        return function (x) { return y.map(function (y) { return [x, y]; }); };
    }

    const graticuleY = (x0, x1, dx) => {
